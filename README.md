...
The site root is a lightweight D1/H4 selector. Detailed reports keep every lifecycle-review result searchable and filterable: the highest-ranked 300 use full inline chart cards, while the remainder use compact cards with on-demand chart links. Complete evaluations also remain available in each timeframe's `results.json`. Published chart filenames include a content hash, so a newly generated image cannot be confused with an older GitHub Pages cache entry. Reports also show the latest candle timestamp as `Data as of`.
